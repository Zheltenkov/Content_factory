...
- **T2.4 Таблицы УП (alembic).** `curriculum_plan` + `curriculum_project` (DB-контракт: `CURRICULUM_ALIAS_FIELD_TO_COLUMN`; CSV-контракт отдельно: `CSV_COLUMN_ALIASES`). Образец: `api/routers/curriculum.py` (алиасы колонок). DoD: миграция, FK, индексы; тест round-trip записи.
...
- **W6 reference/ + UI-финал** (reference ≤1500, static `budget-override:` ≤12000; `vendor/mermaid.min.js` и `land.png` не считаются и не портируются). Свернуть `viewer/app.py` (~6978 в текущем legacy) в тонкий reference-модуль поверх общих таблиц; ver1 `main.js` (~2722) уже опирается на `js/modules/*`, поэтому UI переносится по модульным панелям, а не пилится как ver2.4-монолит. DoD: справочник читается/правится из общей БД; все 5 плиток ведут в живые панели, не в пустые shell-заглушки.
