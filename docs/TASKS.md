...
- **W7 петля + архив.** Reverse-extraction сверяется с каталогом (реконсиляция); архив legacy-репозиториев; включить CI-гейты (AGENTS §CI). DoD: обратная связь замкнута; legacy помечены archived.
