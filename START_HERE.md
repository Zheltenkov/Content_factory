...
- **Волна 2 начата:** T2.1 curriculum catalog pipeline перенесён в `app/modules/curriculum/stages/`,
  ...
  `app/modules/curriculum/repo.py` (SQLAlchemy Core, PG-схема `013`).
- **Дальше:** T2.4 таблицы УП (`curriculum_plan` + `curriculum_project` alembic). Если нужен порт генератора раньше, G1 должен расширять существующий
