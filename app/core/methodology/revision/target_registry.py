"""Stable address registry for scoped human revisions."""
    ...
    def find(self, value: str, *, kind: TargetKind | None = None, stage: str | None = None) -> SectionTarget | None:
        ...
        for target in self.targets:
            ...
            if stage is not None and target.stage != stage:
                continue
            candidates = [target.id, target.selector, target.label, str(target.metadata.get("path") or "")]
            if any(normalized == _norm(candidate) for candidate in candidates):
                return target
        for target in self.targets:
            if kind is not None and target.kind != kind:
                continue
...
def _field_targets(context: dict[str, Any], markdown: str) -> list[SectionTarget]:
    ...
    if str(context.get("title") or "").strip():
        targets.append(
            SectionTarget(
                id="title",
                kind="field",
    ...
    annotation_text = str(annotation.get("text") or "") if isinstance(annotation, dict) else str(getattr(annotation, "text", "") or "")
    has_h1 = bool(markdown and re.search(r"^#\s+.+?$", markdown, flags=re.MULTILINE))
    if annotation_text.strip() and not has_h1:
        targets.append(
            SectionTarget(
                id="annotation",
                kind="field",
...
def _annotation_targets(markdown: str) -> list[SectionTarget]:
    title_match = re.search(r"^#\s+(.+?)\s*$", markdown or "", flags=re.MULTILINE)
    if not title_match:
        return []
    ...
    return [
        SectionTarget(
            ...
            line_start=_line_number(markdown, title_match.end()),
...
def _markdown_targets(markdown: str) -> list[SectionTarget]:
    matches = list(re.finditer(r"^(#{1,6})\s+(.+?)\s*$", markdown or "", flags=re.MULTILINE))
    targets: list[SectionTarget] = []
    ...
    for index, match in enumerate(matches):
        ...
        if level == 2:
            ...
            target_id = _unique_id(f"{chapter_id or stage}.{_subsection_suffix(heading) or _slug(heading)}", used_ids)
    ...
    for index, item in enumerate(dataset_files, 1):
        ...
        targets.append(
            SectionTarget(
                ...
                metadata={"path": path, "index": index, "bytes": _data_size(item.get("data"))},
            )
        )
    return targets
...
def _stage_for_heading(heading: str, fallback: str | None = None) -> str:
    ...
    if "глава 2" in normalized or "теорет" in normalized or re.match(r"^2\.\d+\b", normalized):
        return "theory"
    if "глава 3" in normalized or "практи" in normalized or normalized.startswith("задание ") or "бонус" in normalized:
        return "practice"
    return fallback or "final"


def _chapter_id(heading: str) -> str | None:
    return f"chapter_{match.group(1)}" if (match := re.search(r"глава\s+(\d+)", heading, flags=re.I)) else None


def _subsection_suffix(heading: str) -> str | None:
    normalized = _norm(heading)
    if match := re.search(r"\b2\.(\d+)\b", normalized):
        return f"part_{match.group(1)}"
    if match := re.search(r"\bзадание\s+(\d+)", normalized):
...
def _slug(value: str) -> str:
    text = re.sub(r"[^a-zа-яё0-9]+", "_", value.replace("\\", "/").lower(), flags=re.I)
    return re.sub(r"_+", "_", text).strip("_")[:80] or "target"
