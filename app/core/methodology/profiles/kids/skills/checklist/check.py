"""Kids checklist guard: base YAML/objectivity plus roles/teamwork/communication."""
...
def _objective(text: str, params: dict) -> bool:
...
def check(doc: GeneratedDoc, params: dict) -> list[RuleIssue]:
    ...
    normalized_items = [_norm(text) for text in items]
    for text in items:
        normalized = _norm(text)
        ...
        if not _objective(text, params):
            issues.append(_issue("not_objective", "Пункт чек-листа не даёт однозначного ответа выполнено/нет.", {"text": text}))

    for group, terms in params["required_topic_groups"].items():
        if not any(any(str(term).lower() in item for term in terms) for item in normalized_items):
            issues.append(
                _issue(
                    "missing_kids_topic",
                    "В детском чек-листе должен быть критерий по ролям, командной работе и коммуникации.",
                    {"topic": group, "terms": terms},
                )
            )
    return issues
