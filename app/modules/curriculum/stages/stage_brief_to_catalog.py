...
ACTION_RE = re.compile(
    r"\b(анализ|проектирован|разработ|настрой|тестирован|проведен|проведение|формулирован|управлен|оценк|создан|создание|выбор|интеграц)",
    re.IGNORECASE,
)
TECH_TERMS = ("SQL", "API", "REST", "CI/CD", "Git", "Docker", "LLM", "MVP")
...
def _skill_name(topic: str) -> str:
    ...
    if ACTION_RE.search(text):
        return text[:120]
    return f"Работа с темой «{_short_label(text)}»"


def _short_label(text: str) -> str:
    words = re.sub(r"\s+", " ", text).split()
    return " ".join(words[:8]).strip(" .,-:;") or "общая тема"


def _bloom_for_text(text: str) -> BloomLevel:
    lowered = text.casefold()
    if re.search(r"\b(созда|проектир|разрабатыва|строит|формирует)\b", lowered):
        return "create"
    if re.search(r"\b(анализ|оценива|сравнива|выбира)\b", lowered):
        return "analyze"
    if re.search(r"\b(настраива|применя|использу|проводит|тестиру)\b", lowered):
        return "apply"
    return "understand"
